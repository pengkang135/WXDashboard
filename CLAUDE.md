...
分类体系随项目而定。关键词规则无法匹配的群自动归入"其他"标签，显示在外部分类末尾。用户可拖拽到正确分类，或运行 Claude Code AI 分类。

当需要 AI 分类时:

1. 调用 `get_groups_for_classification()` 获取待分类群 + 最近 5 条样本消息（从 SQLite 读取）
2. 根据当前项目特点判断分类。工程项目典型分类:总承包（内部沟通、施工局合作、设计院合作）、各分包（供应商询价、地基处理、建筑MEP、保险、物流、其他）
3. 通过 `/api/groups/<id>/settings` 写入 `category` 和 `sub_category`
4. 如判断不确定，留在"其他"标签并给出理由，不强行分类
5. 群间顺序处理，逐个判断
...
2. 技能自动完成全部流程:
   ...
   - 按五个维度提取: 联系人、工期节点、技术参数、文件引用、专业/供货类别
   - 审核提取质量，有问题则补充
...
- **关键信息**: 分类展示提取的结构化信息（联系人、技术参数、工期、文件引用、专业/供货类别）。文件引用支持点击跳转文件位置。联系人显示格式为"微信名（真实称呼）"
