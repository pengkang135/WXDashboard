...
![alt text](docs/intro/全部页.png)

## 架构
```mermaid
flowchart LR
    A[微信加密DB] -->|wx-cli| B[sync_engine]
    B --> C[(SQLite + FTS5)]
    C --> D[Flask API]
    D --> E[浏览器 SPA]
```
...
flowchart TD
    subgraph 前端
        SPA[dashboard.html + app.js + style.css]
    end

    subgraph 后端
        Flask["Flask app.py<br/>15 REST 端点"]
        Sync[sync_engine.py<br/>wx-cli 调用 / 去重 / 类别推断]
        Extractor[contact_extractor.py<br/>正则提取联系信息]
    end
...
```mermaid
flowchart TD
    A[微信 MSG.db 加密] -->|wx sessions --json| B[JSON 消息数组]
    B -->|_upsert_from_wx_msg| C{去重<br/>group_id + local_id}
    C -->|extract_and_save| D[正则提取 → contacts]
    C -->|upsert_message| E[messages + FTS5 触发器]
    D --> F[Flask REST API]
    E --> F
    F -->|JSON| G[浏览器渲染]
```
...
| `backend/app.py`               | 15 个 REST 端点，Flask 开发服务器          |
...
erDiagram
    ...
    groups ||--o{ contacts : "group_id"
    messages ||--o| messages_fts : "FTS5 trigger"
